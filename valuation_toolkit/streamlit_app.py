...
with st.sidebar:
    ...
    manual_peers = st.text_input(
        'Manual peer override (comma-separated)',
        value='MSFT,GOOGL,META,AMZN,ORCL,ADBE,CRM,INTU' if ticker == 'AAPL' else '',
    )
    ...
    try:
        with st.spinner('Pulling live data and building valuation...'):
            ...
            if manual_peers.strip():
                manual_symbols = [x.strip().upper() for x in manual_peers.split(',') if x.strip()]
                peer_snapshots = []
